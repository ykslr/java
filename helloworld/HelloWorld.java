...
//     对象、数组都是引用数据类型
//     所有引用类型的默认值都是null
//     一个引用变量可以用来引用任何与之兼容的类型，父类的引用可以指向子类对象
...
/* Java 数组 */
// 声明数组：Type[] var; 或者 Type var[]; 建议使用第一种
// 使用 new 操作符来创建数组： var = new Type[];


/* Java 日期时间 */
// java.util 包提供了 Date 类来封装当前的日期和时间。 Date 类提供两个构造函数来实例化 Date 对象。
//     第一个构造函数使用当前日期和时间来初始化对象：Data()
//     第二个构造函数接收一个参数，该参数是从1970年1月1日起的毫秒数：Date(long millisec)
//     可以使用 SimpleDateFormat 格式化日期，也可以将字符串解析为时间
//     Java 休眠：sleep，以毫秒为单位
// Calendar 类
//     Calendar类的功能要比Date类强大很多，而且在实现方式上也比Date类要复杂一些
//     Calendar类是一个抽象类，在实际使用时实现特定的子类的对象，创建对象的过程对程序员来说是透明的，只需要使用getInstance方法创建即可
//     比如创建一个代表系统当前日期的 Calendar 对象：Calendar c = Calendar.getInstance(); 默认为当前日期
//     可以设定该对象中的时间：c.set(2008, 8, 8);
...
//    {n}: n为非负整数。正好匹配n次
//    {n,}: n为非负整数。至少匹配n次
//    {n,m}: n, m为非负整数，且n<=m。至少匹配n次，至多m次
//    *: 零次或多次匹配前面的字符或子表达式，相当于{0,}
//    +: 一次或多次匹配前面的字符或子表达式，相当于{1,}
//    ?: 零次或一次匹配前面的字符或子表达式，相当于{0,1}
//    .: 匹配除"\r\n"之外的任何单个字符
//    (pattern): 按照括号里的表达式匹配并捕获
//    (?：pattern): 按照括号里的表达式匹配但不捕获
//    [xyz]: 匹配包含的任一字符
//    [^xyz]: 匹配未包含的任一字符
//    [a-z]: 匹配字符范围内的任一字符
//    [^a-z]: 匹配字符范围外的任一字符
...
/* Java 方法 */
// 方法可以重载，要保证拥有不同的参数列表
// Java的构造方法和它的类的名字相同，且没有返回值
// Java的可变参数的形式为: Type... parameterName。注意 ... 是紧挨着变量类型的
// Java的 finalize() 方法，在对象被垃圾收集器析构(回收)之前调用，它用来清除回收对象
